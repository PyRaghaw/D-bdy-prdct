...
function FamilyCareNetwork() {
  return (
    <div className="relative h-[390px] sm:h-[480px] md:h-[520px] overflow-hidden rounded-[36px] border border-white/10 bg-black/45 shadow-[0_40px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl">
      ...
      {/* Central Hub */}
      <div className="absolute left-1/2 top-1/2 z-20 flex h-20 w-20 sm:h-28 md:h-36 sm:w-28 md:w-36 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-brand/40 bg-brand/15 shadow-[0_0_70px_rgba(92,96,245,0.36)] backdrop-blur-xl">
        ...
        <div className="text-center">
          <div className="mx-auto mb-0.5 sm:mb-2 flex h-7 w-7 sm:h-10 md:h-14 sm:w-10 md:w-14 items-center justify-center rounded-full bg-white text-[11px] sm:text-[15px] md:text-[20px] shadow-lg">🏥</div>
          <div className="font-[family-name:var(--font-bricolage)] text-[9px] sm:text-xs md:text-sm font-black text-white leading-tight">Patient Plan</div>
          <div className="mt-0.5 text-[6px] sm:text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-white/45 hidden sm:block">AI hub</div>
        </div>
      </div>
      ...
      {careNodes.map((node) => (
        ...
        >
          <div
            className="relative flex h-10 w-10 sm:h-14 md:h-20 sm:w-14 md:w-20 items-center justify-center rounded-full border-2 border-white/40 text-white shadow-[0_18px_50px_rgba(0,0,0,0.25)] transition-all duration-300 group-hover:scale-110"
            style={{ background: `linear-gradient(135deg,${node.color},#5c60f5)` }}
          >
            <span className="absolute inset-[-4px] sm:inset-[-6px] md:inset-[-8px] rounded-full opacity-30 blur-md" style={{ backgroundColor: node.color }} />
            <span className="relative font-[family-name:var(--font-bricolage)] text-xs sm:text-sm md:text-lg font-black">{node.initials}</span>
          </div>
          <div className="mt-1 sm:mt-2 min-w-[70px] sm:min-w-[100px] md:min-w-[150px] rounded-lg sm:rounded-2xl border border-white/10 bg-white/[0.075] px-1 sm:px-2 md:px-3 py-1 sm:py-1.5 md:py-2 text-center backdrop-blur-xl transition-all duration-300 group-hover:bg-white/[0.12]">
            <div className="font-[family-name:var(--font-bricolage)] text-[9px] sm:text-[11px] md:text-[13px] font-black text-white">{node.role}</div>
            <div className="mt-0.5 text-[7px] md:text-[10px] leading-snug text-white/45 hidden md:block">{node.note}</div>
          </div>
        </div>
      ))}
    </div>
